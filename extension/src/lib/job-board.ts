// Board detection + extraction dispatch, shared by contents/detect.ts (popup's
// EXTRACT_JOB message flow) and lib/widget (the floating widget's own
// detection). Lives in lib/, not contents/, so importing it never drags in
// detect.ts's onMessage listener as a side effect — Plasmo treats every
// top-level contents/ file as its own content-script entry.
